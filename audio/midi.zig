...
pub const MidiReader =
    if (builtin.os.tag == .windows) @import("windows/MidiReader.zig") else @compileError("Unuspported OS");
...
fn getScale(octave: i5) f32 {
    if (octave >= 0) {
        return pos_scale_table[@intCast(octave)];
    }
    return neg_scale_table[@intCast(-octave)];
}
...
// }
pub const MidiMsg = packed struct(u24) {
    status_arg: u4,
    kind: MidiMsgKind,
    msb_status: u1,
    // status: Status,
    ...
    comptime {
        std.debug.assert(@sizeOf(Data) == 2);
    }
...
pub const MidiStreamProcessor = struct {
    ...
    pub fn process(self: *MidiStreamProcessor, data: []const u8) ProcessError!ProcessResult {
        ...
        const full_len = @as(u3, info.status_len) + data_len;
        ...
        //         1 => [3]u8{ info.status, data[info.status_len], undefined },
        //         2 => [3]u8{ info.status, data[info.status_len], data[@as(usize, info.status_len) + 1] },
        //         else => unreachable,
        //     } }).msg,
        // };
    }
...
pub fn checkMidiMsg(msg: MidiMsg) !void {
    ...
    if (data_len >= 1 and (std.mem.asBytes(&msg.data)[0] & 0x80) != 0) return error.MidiMsgFirstDataByteMsbIsOne;
    if (data_len >= 2 and (std.mem.asBytes(&msg.data)[1] & 0x80) != 0) return error.MidiMsgSecondDataByteMsbIsOne;
}
...
pub fn logMidiMsg(msg: MidiMsg) void {
    switch (msg.kind) {
        .note_off, .note_on => {
            ...
            midilog.debug("note_{s} channel={} note={s}({}) velocity={}", .{ kind, msg.status_arg, @tagName(@as(MidiNote, @enumFromInt(msg.data.note_off.note))), msg.data.note_off.note, msg.data.note_off.velocity });
        },
        .poly_pressure => {
            midilog.debug("poly_pressure channel={} {}", .{ msg.status_arg, msg.data.poly_pressure });
        },
        .control_change => {
            midilog.debug("control_change channel={} num={} velocity={}", .{ msg.status_arg, msg.data.control_change.num, msg.data.control_change.velocity });
        },
        .program_change => {

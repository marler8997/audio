...
pub fn Template(comptime Format: type) type {
    return struct {
        ...
        pub const Mix = struct {
            ...
            const SampleIterator = struct {
                ...
                pub fn next(self: *SampleIterator) ?SampleRef {
                    ...
                    const ref = SampleRef{ .channels = self.mix.channels, .buffer = self.next_buffer };
        ...
        const Knob = union(enum) {
            ...
            float_cb: struct {
                min: f32,
                max: f32,
                cb: *const fn (usize, *f32, Access) void,
            },
            sample_field: struct {
                field_offset: u32,
            },
            ...
            pub fn setF32(self: Knob, context: usize, value: f32) void {
                switch (self) {
                    .float_field => |k| @as(*f32, context + k.field_offset).* = std.math.clamp(value, k.min, k.max),
                    .float_cb => |k| {
                        var set_value = std.math.clamp(value, k.min, k.max);
                        k.cb(context, &set_value, .set);
                    },
                    .sample_field => |k| @as(*Sample, context + k.field_offset).* = Format.f32ToSample(value),
                }
            }
        };
        ...
        pub const singlestep = struct {
            ...
            pub fn Chain(comptime Generator: type, comptime filter_types: []const type) type {
                // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                comptime {
                    const fields = @typeInfo(std.meta.Tuple(filter_types)).@"struct".fields;
                    for (fields) |f| {
                        if (f.is_comptime)
                            @compileError(std.fmt.comptimePrint(
                                "field '{s}': type={}, is_comptime={}",
                                .{ f.name, f.type, f.is_comptime },
                            ));
                    }
                }
                return struct {
            ...
            pub fn AttachedKnob(comptime Component: type, comptime KnobChanger: type, comptime knob: Knob) type {
                _ = knob;
                return struct {
                    const Self = @This();
            ...
            pub fn TrigUnitToFullSampleGenerator(comptime TrigUnitGenerator: type) type {
                return struct {
                    ...
                    pub fn setFreq(self: *@This(), freq: f32) void {
                        self.unit_generator.setFreq(freq);
                    }
                };
            }
            ...
            pub fn CombFilter(comptime FeedforwardDelay: comptime_int, comptime FeedbackDelay: comptime_int) type {
                return struct {
            ...
            pub fn MidiVoice(comptime Renderer: type) type {
                return struct {
                    renderer: Renderer,
            ...
            pub const NoteFreqF32KnobChanger = struct {
                ...
                pub fn init(opt: InitOptions) NoteFreqF32KnobChanger {
                    return .{ .event_sample_time = opt.event_sample_time, .event_tick = 0, .note_start = opt.note_start, .note_end = opt.note_end, .note_inc = opt.note_inc, .note = opt.note_start };
                }
                ...
                pub fn nextSample(self: *@This(), knob: Knob, knob_context: usize) void {
                    if (self.event_tick == self.event_sample_time) {
                        ...
                        if (next_note >= @intFromEnum(self.note_end)) {
                            ...
                            self.note = @enumFromInt(@as(u7, next_note));
                        }
            ...
            pub fn renderGenerator(comptime T: type, generator: *T, out: Mix) void {
                ...
                while (it.next()) |sample| {
                    sample.add(T.renderOneSample(generator));
                }
            }
        };
    };
...
test "saw" {
    ...
    var soft_saw = S.singlestep.Volume(S.singlestep.Saw){ .volume = 0.1, .forward = .{
        .next_sample = 0,
        .increment = 0.2,
    } };
    buffer = [_]f32{0} ** 7;

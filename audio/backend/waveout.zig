...
fn writeBuffer(renderBuffer: [*]const SamplePoint) anyerror!void {
    ...
    @memcpy(
        global.backBuffer.base.lpData.?,
        @as([*]const u8, @ptrCast(renderBuffer))[0 .. audio.global.bufferSampleFrameCount * audio.global.channelCount * @sizeOf(SamplePoint)],
    );
    ...
    {
        const result = win32.waveOutPrepareHeader(global.waveOut, &global.backBuffer.base, @sizeOf(@TypeOf(global.backBuffer.*)));
        if (result != win32.MMSYSERR_NOERROR) {
    ...
    if (0 == win32.ResetEvent(global.backBuffer.freeEvent)) {
        audiooutlog.err("ResetEvent failed, e={f}", .{win32.GetLastError()});
        return error.Unexpected;
    }
    {
        const result = win32.waveOutWrite(global.waveOut, &global.backBuffer.base, @sizeOf(@TypeOf(global.backBuffer.*)));
        if (result != win32.MMSYSERR_NOERROR) {
    ...
    switch (win32.WaitForSingleObjectEx(global.frontBuffer.freeEvent, win32.INFINITE, win32.FALSE)) {
        @intFromEnum(win32.WAIT_OBJECT_0) => {},
            ...
            audiooutlog.err("WaitForSingleObjectEx failed, result={}, error={f}", .{ err, win32.GetLastError() });
            return error.WaitForSingleObjectFailed;
        },
    }
    // TODO: is unprepare necessary with no backbuffer?
    {
        const result = win32.waveOutUnprepareHeader(global.waveOut, &global.frontBuffer.base, @sizeOf(@TypeOf(global.backBuffer.*)));
        if (result != win32.MMSYSERR_NOERROR) {
            audiooutlog.err("waveOutUnprepareHeader failed, result={}", .{result});
            return LocalError.WaveOutUnprepareHeaderFailed;
        }
    }
    const temp = global.frontBuffer;
...
fn setupGlobalData() anyerror!void {
    ...
    for (&global.waveHeaders) |*waveHeader| {
        ...
        const buffer = try audio.global.allocator.alloc(SamplePoint, global.playBufferSize);
        ...
        waveHeader.freeEvent = win32.CreateEventA(null, 1, 1, null) orelse {
            audiooutlog.err("CreateEventA failed, error={f}", .{win32.GetLastError()});
...
pub fn waveOutCallback(
    ...
    if (msg == win32.MM_WOM_OPEN) {
        ...
        const header: *CustomWaveHeader = @alignCast(@fieldParentPtr("base", @as(*win32.WAVEHDR, @ptrCast(param1))));
